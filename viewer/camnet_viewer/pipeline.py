...
Usa decodebin3 como decoder principal (HW acceleration cuando disponible).
Si decodebin3 falla con "No caps set" o "Broken bit stream", el CameraPlayer
reintenta con decodebin (fallback). Este es el único fallback; no hay heurísticas
adicionales.
"""
...
def build_pipeline(
    ...
    if src is None:
        raise RuntimeError("rtspsrc no disponible")
    ...
    if sink is None:
        raise RuntimeError("gtk4paintablesink no disponible")
